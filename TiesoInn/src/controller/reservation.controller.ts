...
class ReservationController {
  ...
  async createReservation(req: Request, res: Response) {
    ...
    try {
      ...
      const roomObjectId = mongoose.Types.ObjectId.isValid(room_id)
        ? new mongoose.Types.ObjectId(room_id)
        : null;
      const roomExists = roomObjectId
        ? await Room.findById(roomObjectId)
        : null;
      ...
      if (!userExists || !roomExists) {
        ...
        return res
          .status(HTTP_STATUS_CODES.NOT_FOUND)
          .json({ message: "Usuario o habitación no válidos" });
      }

      // Buscamos todos los números de reservación y los almacenamos en un array
      const allReservationNumbers = await Reservation.find({})
        .select("reservation_num")
        .lean();
      const usedReservationNumbers = allReservationNumbers.map(
      ...
      res
        .status(HTTP_STATUS_CODES.SERVER_ERROR)
        .json({ message: "Error al crear la reserva" });
    }
  }
  ...
  async updateReservation(req: Request, res: Response) {
    ...
    try {
      ...
      res
        .status(HTTP_STATUS_CODES.SERVER_ERROR)
        .json({ message: "Error al actualizar la reservacion", error });
    }
  }
  ...
  async deleteReservation(req: Request, res: Response) {
    ...
    try {
      ...
      res
        ...
        .json({ message: "Reserva Eliminada Correctamente" });
    } catch (error) {
      res
        .status(HTTP_STATUS_CODES.SERVER_ERROR)
        .json({ message: "Error al eliminar la reserva", error });
    }
  }
}
